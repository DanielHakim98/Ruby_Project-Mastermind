...
def check_input(guess_str)
  range = guess_str.none? { |item| item < 1 || item > 8 }
  until guess_str.length == 4 && range == true
...
def check_duplicate(guess_str)
  ...
  until duplicate == true && range == true && guess_str.length == 4
    puts "\nInvalid Input."
    puts "Please re-enter input again (1-8) 4 digit only."
    puts"No duplicate!: "
    guess_str = gets.chomp.split('').map { |item| item.to_i }
    range = guess_str.none? { |item| item < 1 || item > 8 }
    duplicate = guess_str.uniq.length == guess_str.length
...
def show_previous(previous_indicator, previous_select, guess_str)
...
def code_breaker_start(guess_this)
  red_clue = Array.new
  ...
  10.times do |count|
    ...
    show_previous(previous_indicator, previous_select, guess_str)
...
code_breaker_start(guess_this) if choice == 1
